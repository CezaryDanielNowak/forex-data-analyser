...
const DEFAULT_BUNDLE = `app-${config.LANGUAGE}.js`;
...
function getBundle(fileName) {
  try {
    ...
    if (config.ENV.substr(0, 5) === 'local') {
      setTimeout(() => {
...
function getAllRoutes(bundleName) {
  const bundle = getBundle(bundleName || DEFAULT_BUNDLE);
  const routes = bundle['./routes'];

  const result = [];
  const crawlRoutes = (children) => {
    if (children instanceof Array) {
      return children.forEach(crawlRoutes);
    }
    ...
    if (children.props.children) {
      if (children.props.children instanceof Array) {
        children.props.children.forEach(crawlRoutes);
      } else {
        // one child
        crawlRoutes(children.props.children);
      }
    }
  ...
  const tree = routes.props.children instanceof Array ? routes.props.children : [routes.props.children];

  tree.forEach(crawlRoutes);

  return result;
}

function getAllPages(bundleName) {
  const allRoutes = getAllRoutes(bundleName);
  const result = {};

  const promises = allRoutes.map((location) => {
    return generateMarkup(location, bundleName || DEFAULT_BUNDLE)
      .then((resultObj) => {
        gutil.log('GENERATE: ', `(${bundleName.replace(/(app-|\.js)/g, '')}) ${location}`);
